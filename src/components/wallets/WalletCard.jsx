...
/**
 * Props:
 * - wallet: { id, walletName, initialBalance, currencyCode, createdAt, setAsDefault, isShared,
 *             includeOverall?, includePersonal?, includeGroup? }
 * - onToggleOverall(wallet, nextOn)
 * - onToggleSection(wallet, nextOn)
 */
  ...
  return (
    <div
      className={`wallet-card card wc--flat ${
        wallet.setAsDefault ? "wallet-card--default" : ""
      }`}
    >
      {/* ================= Scoped CSS ================= */}
      <style>{`
        ...
        .wallet-card .card-body{ padding: 44px 16px 16px; }
      ...
      <div className="card-body" onClick={stop}>
        <div className="d-flex align-items-center justify-content-between mb-2">
          <h6 className="wallet-name mb-0">
            {wallet.walletName}
            {wallet.setAsDefault && (
              <span className="badge ms-2">Mặc định</span>
            )}
          </h6>
        </div>
        ...
        <div className="wc-body">
          <div className="wc-row">
            ...
            <span className="wc-value text-primary">
              {fmtMoney(wallet.initialBalance, wallet.currencyCode)}
            </span>
          </div>
