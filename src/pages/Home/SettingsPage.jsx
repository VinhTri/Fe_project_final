...
export default function SettingsPage() {
  ...
  const loadProfile = async () => {
    try {
      ...
      console.error(err);
      const msg =
        err.data?.message || // ✅ sửa: dùng err.data
        err.data?.error ||
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      if (isGoogleFirstPassword) {
        // 🔐 Tài khoản Google lần đầu → newPassword + confirmPassword
        await setFirstPassword({ newPassword, confirmPassword }); // ✅ thêm confirmPassword
        setSuccess("Thiết lập mật khẩu lần đầu thành công");

        // cập nhật lại user để ẩn flow "lần đầu"
        setUser((prev) =>
          prev ? { ...prev, firstLogin: false } : prev
        );
      } else {
        // 🔐 Tài khoản thường → oldPassword + newPassword + confirmPassword
