...
const mapBackendToFrontend = (backendWallet) => {
  // Log để debug
  if (!backendWallet) {
    console.warn("WalletDataContext: mapBackendToFrontend nhận null/undefined wallet");
    return null;
  }
...
export function WalletDataProvider({ children }) {
  ...
  const createWallet = async (payload) => {
    try {
      ...
      console.log("WalletDataContext: Creating wallet với payload:", backendPayload);
      const { response, data } = await createWalletAPI(backendPayload);
      console.log("WalletDataContext: Create wallet response:", { response, data });
      ...
      if (response.ok && data.wallet) {
        ...
        const mergedWallet = {
          ...
          includeOverall: payload.includeOverall !== undefined ? payload.includeOverall : true,
          includePersonal: payload.includePersonal !== undefined ? payload.includePersonal : true,
          includeGroup: payload.includeGroup !== undefined ? payload.includeGroup : true,
        ...
        if (mergedWallet.isShared && mergedWallet.groupId) {
          setGroups(prev => prev.map(g => g.id === mergedWallet.groupId
            ? { ...g, walletIds: Array.from(new Set([...(g.walletIds||[]), mergedWallet.id])) }
            : g
          ));
        }
    ...
    try {
      ...
      if (Object.keys(backendPayload).length > 0) {
        ...
        if (response.ok && data.wallet) {
          ...
          setWallets(prev => prev.map(w => (w.id === walletId ? mergedWallet : w)));
          return mergedWallet;
        } else {
        ...
        setWallets(prev => prev.map(w => (w.id === walletId ? { ...w, ...patch } : w)));
        return { ...patch };
      }
    } catch (error) {
